...
bool verifyBackendReplacementPublishesOneQmlRefresh(QTextStream& err)
{
    ...
    const QString documentModel = sourceFile(
        QStringLiteral("src/app/ui/document/DocumentModel.cpp"));
    const QString mainView = sourceFile(QStringLiteral("src/app/ui/layout/MainView.qml"));
    const QString mainSplitView = sourceFile(
        QStringLiteral("src/app/ui/layout/MainSplitView.qml"));
    const QString editorPane = sourceFile(
        QStringLiteral("src/app/ui/editor/EditorPane.qml"));
    const QString viewState = sourceFile(QStringLiteral("src/app/ui/ViewState.qml"));
    ...
    const QString documentBridge = sourceFile(QStringLiteral("src/app/services/DocumentBridge.h"));
    ...
    return require(mainWindow.contains(QStringLiteral("void documentReplaced();")),
        ...
        && require(containsAfter(documentModel,
                                 QStringLiteral("&miacode::ShellNotifications::documentReplaced"),
                                 QStringLiteral("adoptBackendDocumentReplacement();"))
                       && containsAfter(documentModel,
                                        QStringLiteral("&miacode::ShellNotifications::documentReplaced"),
                                        QStringLiteral("Qt::QueuedConnection"))
                       && containsAfter(documentModel,
                                        QStringLiteral("void DocumentModel::adoptBackendDocumentReplacement()"),
...
bool verifyQmlNewDocumentUsesRequestServices(QTextStream& err)
{
    const QString commandService = sourceFile(
        QStringLiteral("src/app/ui/document/CommandService.cpp"));
    const QString documentModel = sourceFile(
        QStringLiteral("src/app/ui/document/DocumentModel.cpp"));
    const QString fileFlow = sourceFile(
        QStringLiteral("src/app/runtime/document/DocumentFileFlow.cpp"));
    return require(containsAfter(commandService,
                                 QStringLiteral("void CommandService::newDocument()"),
        ...
        && require(containsAfter(documentModel,
                                 QStringLiteral("void DocumentModel::createDocumentFromPickedAudio()"),
        ...
        && require(containsAfter(documentModel,
                                 QStringLiteral("void DocumentModel::createEmptyDocumentAt"),
...
bool verifyV2AnalysisUsesWorkspaceSnapshot(QTextStream& err)
{
    const QString applicationContext = sourceFile(
        QStringLiteral("src/app/ui/ApplicationContext.cpp"));
    const QString applicationServices = sourceFile(
        QStringLiteral("src/app/services/ApplicationServices.h"));
    const QString analysisServiceHeader = sourceFile(
        QStringLiteral("src/app/services/AnalysisService.h"));
    const QString analysisService = sourceFile(
        QStringLiteral("src/app/services/AnalysisService.cpp"));
    const QString documentModel = sourceFile(
        QStringLiteral("src/app/ui/document/DocumentModel.cpp"));
    const QString analysisModel = sourceFile(
        QStringLiteral("src/app/ui/document/AnalysisModel.cpp"));

    // Stage 3.5 item 1 moved the single analysis service from the QML context to
    // miacode::ApplicationServices. The contract is unchanged in substance —
    ...
    return require(applicationServices.contains(QStringLiteral("AnalysisService analysis_;"))
        ...
        && require(documentModel.contains(QStringLiteral("analysisService_->snapshot()"))
                       && analysisService.contains(QStringLiteral("void AnalysisService::requestAnalysis()"))
...
bool verifyWorkspaceOwnsProductionDocumentAndDirty(QTextStream& err)
{
    ...
    return require(
        ...
        && require(documentModel.contains(
                       ...
                       && documentModel.contains(QStringLiteral("saveSectionDifficultyId()"))
                       && documentModel.contains(QStringLiteral("fileService_->save(sectionId)")),
        ...
        && require(documentModel.contains(
                       QStringLiteral("void DocumentModel::saveSectionOrAskForPath"))
                       && documentModel.contains(QStringLiteral("request.saveMode = true"))
                       && documentModel.contains(QStringLiteral("saveSectionOrAskForPath(difficultyId,"))
                       && documentModel.contains(QStringLiteral("const bool wholeDocument = difficultyId == 0")),
        ...
        && require(!documentModel.contains(QStringLiteral("backend_->isWindowModified()"))
                   ...
                   QStringLiteral("DocumentModel never asks MainWindow to own a mutation or determine dirty"), err)
        ...
        && require(!documentModel.contains(QStringLiteral("applyCommittedDocument("))
                       && frameBootstrap.contains(QStringLiteral("&miacode::ChartWorkspace::changed"))
        ...
        && require(timelineFlow.contains(
                       QStringLiteral("updateDocumentField(miacode::ChartWorkspaceDocumentField::First"))
...
bool verifyPageNavigationUsesTheQmlLeaveGuard(QTextStream& err)
{
    ...
    const QString pageHost = sourceFile(
        QStringLiteral("src/app/ui/layout/PageHost.cpp"));
    const QString pageHostHeader = sourceFile(
        QStringLiteral("src/app/ui/layout/PageHost.h"));
    const QString documentPages = sourceFile(
        QStringLiteral("src/app/runtime/document/DocumentPages.cpp"));
    const QString documentFileFlow = sourceFile(
        QStringLiteral("src/app/runtime/document/DocumentFileFlow.cpp"));
    ...
    ok &= require(documentModel.contains(
                       QStringLiteral("void DocumentModel::requestLeaveCurrentField"))
